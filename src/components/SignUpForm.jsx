...
function SignUpForm({setAuthToken}){
  ...
  return(
   <div className="signup">
    ...
    {successMessage&&<p className="success">{successMessage} You can go ahead and generate your event token below.</p>}
    <form onSubmit={handleSubmit}>
      <div className="input">
      <label >User Name: <input value={username} onChange={handleUserName} maxLength={8} minLength={8} required/></label>
      <label >Password: <input value={password} onChange={handlePassword} type="password" required/></label>
      </div>
      <button className="button" disabled={(username&&password)? false:true}>Submit</button>
    </form>
  </div>
  )
 }
